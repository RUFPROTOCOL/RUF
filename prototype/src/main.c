...
struct rtp_controller controller;
	...
	while(!force_quit) {
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
				p = pkts_burst[j];
				// rte_vlan_strip(p);
				rtp_rx_packets(&receiver, &sender, &pacer, p);
			}
		}
		cur_tsc = rte_rdtsc();
...
static void pacer_main_loop(void) {
	...
	rte_timer_reset(&pacer.token_timer, 0, SINGLE,
        rte_lcore_id(), &rtp_pacer_send_token_handler, (void *)pacer.send_token_timeout_params);

	rte_timer_reset(&pacer.data_timer, 0, SINGLE,
    	rte_lcore_id(), &rtp_pacer_send_data_pkt_handler, (void *)pacer.send_data_timeout_params);
	...
	while(!force_quit){
		...
		while(!rte_ring_empty(pacer.ctrl_q)) {
			...
			if(rtp_hdr->type == RTP_LISTSRCS) {
				struct rtp_listsrc_hdr *rtp_listsrc_hdr = rte_pktmbuf_mtod_offset(p, struct rtp_listsrc_hdr*, 
					sizeof(struct ether_hdr) + sizeof(struct ipv4_hdr) + sizeof(struct rtp_hdr));
				if(debug_flow(rtp_listsrc_hdr->has_nrts)) {
					printf("send nrts for %u\n", rtp_listsrc_hdr->num_srcs);
				}
			}
			// rte_vlan_insert(&p); 
	...
	while(!force_quit) {
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
				p = pkts_burst[j];
				// rte_vlan_strip(p);
				rtp_receive_listsrc(&controller, p);
			}
		}
		rte_timer_manage();
	}
...
static void flow_generate_loop(void) {
   ...
   //      	if(i == 0) {
			// 	receiver.start_cycle = rte_get_timer_cycles();
   //      	}
			// rtp_new_flow_comes(&sender, & pacer, i, params.dst_ip, 1460 * 1000);
   //      	i++;
   //          prev_tsc = cur_tsc;
   //      }
        if(cur_tsc - prev_tsc_2 > diff_tsc_2) {
			receiver.end_cycle = rte_get_timer_cycles();
			double time = (double)(receiver.end_cycle - receiver.start_cycle) / (double)rte_get_tsc_hz();
			double sent_tpt = (double)(sender.sent_bytes) * 8 / time;
			double receive_tpt = (double)(receiver.received_bytes) * 8 / time;
